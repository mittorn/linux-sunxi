...
#define DMARD_IOCTL_BASE		0xea

#define IOCTL_SENSOR_SET_DELAY_ACCEL		_IO(DMARD_IOCTL_BASE, 0x64)
#define IOCTL_SENSOR_GET_DELAY_ACCEL		_IO(DMARD_IOCTL_BASE, 0x65)
#define IOCTL_SENSOR_GET_STATE_ACCEL		_IO(DMARD_IOCTL_BASE, 0x66)
#define IOCTL_SENSOR_SET_STATE_ACCEL		_IO(DMARD_IOCTL_BASE, 0x67)
#define IOCTL_SENSOR_GET_DATA_ACCEL		_IO(DMARD_IOCTL_BASE, 0x68)

#define IOCTL_MSENSOR_SET_DELAY_MAGNE		_IO(DMARD_IOCTL_BASE, 0xc8)
#define IOCTL_MSENSOR_GET_DATA_MAGNE		_IO(DMARD_IOCTL_BASE, 0xc9)
#define IOCTL_MSENSOR_GET_STATE_MAGNE		_IO(DMARD_IOCTL_BASE, 0xca)
#define IOCTL_MSENSOR_SET_STATE_MAGNE		_IO(DMARD_IOCTL_BASE, 0xcb)

#define IOCTL_SENSOR_GET_NAME   _IO(DMARD_IOCTL_BASE, 0x12d)
#define IOCTL_SENSOR_GET_VENDOR   _IO(DMARD_IOCTL_BASE, 0x12e)

#define IOCTL_SENSOR_GET_CONVERT_PARA   _IO(DMARD_IOCTL_BASE, 0x191)
...
static int dmard06_chip_init(struct dmard06_data *sensor_data)
{
	...
	pr_info("i2c Read SW_RESET = %x\n", cData);

	cAddress = WHO_AM_I;
	i2c_master_send(sensor_data->client, (char *)&cAddress, 1);
	i2c_master_recv(sensor_data->client, (char *)&cData, 1);
	pr_info("i2c Read WHO_AM_I = %d\n", cData);

	if ((cData&0x00FF) == WHO_AM_I_VALUE)
		pr_info("dmard06 gsensor registered I2C driver!\n");
	else {
		pr_info("dmard06 gsensor I2C err = %d!\n", cData);
		return -1;
	}
...
static int dmard06_i2c_read_xyz(struct dmard06_data *data, u8 *pX, u8 *pY, u8 *pZ)
{
	...
	if (ret < 0) {
		pr_err("dmard06_i2c_read_xyz err->%d\n", ret);
		return -EAGAIN;
	}
...
static void dmard06_early_suspend(struct early_suspend *handler)
{
	...
	/* pr_info("dmard06_early_suspend 2\n"); */
...
static void dmard06_early_resume(struct early_suspend *handler)
{
	...
	/* pr_info("dmard06_early_resume 2\n"); */
...
static int gsensor_fetch_sysconfig_para(void)
{
	...
	if (SCRIPT_PARSER_OK != script_parser_fetch("gsensor_para", "gsensor_used", &device_used, 1)) {
	...
	if (1 == device_used) {
		...
		pr_info("%s: after: gsensor_twi_addr is 0x%x, dirty_addr_buf: 0x%hx. dirty_addr_buf[1]: 0x%hx\n",
		...
		pr_info("%s: twi_id is %d.\n", __func__, twi_id);

		dmard06_pin_hd = gpio_request_ex("gsensor_para", NULL);
		if (dmard06_pin_hd == -1)
			pr_err("dmard06 pin request error!\n");
		ret = 0;
